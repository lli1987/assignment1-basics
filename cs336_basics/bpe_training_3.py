...
def _process_pretoken_count(
    input_paths, num_counter_processes, num_merge_processes, special_tokens, num_chunks
):
    ...
    for input_path in input_paths:
        for chunk in _chunk_doc_streaming(
            input_path=input_path, num_chunks=num_chunks, special="<|endoftext|>"
        ):
...
def train_bpe(
    ...
    serialize_bpe(special_tokens, prefix_with_name(name, SPECIAL_TOKENS))
    return vocab, merges
...
def print_diffs(pair_counts_1, pair_counts_2):
    diffs_1, diffs_2, diffs = [], [], []
    for pair, cnt in pair_counts_1.items():
        if pair not in pair_counts_2 and cnt != 0:
    ...
    for pair, cnt in pair_counts_2.items():
        if pair not in pair_counts_1 and cnt != 0:
            print(f"{pair}: pair missing in former, the count is {cnt}")
            diffs_2.append(pair)
    return diffs_1, diffs_2, diffs
        ...
        while idx < len(affected_pre_token):
            if (
                ...
                if idx + 2 < len(affected_pre_token):
                    ...
                    next_is_merge = (idx + 3 < len(affected_pre_token)
...
def merge(
    ...
    pair_counts, pair_to_pre_token_set = build_pair_counts_and_pair_to_pre_tokens(
        pre_tokens_count
    )
...
if __name__ == "__main__":
    ...
    # input_file = "/Users/luyaoli/code/cs336/assignment1-basics/tests/fixtures/corpus.en"
    # vocab, merges = train_bpe(
    #     name="test",
    #     input_paths=[input_file],
    #     vocab_size=300,
    #     special_tokens=["<|endoftext|>"],
    #     num_counter_processes=1,
